...
let reset_time = new Date().setHours(
  reset_time_info.hours,
  reset_time_info.minutes
);
        ...
        chrome.storage.sync.get(
          ...
          stored_data => {
            try {
              ...
              if (
                stored_data.codeforces == undefined ||
                JSON.stringify(stored_data.date) != JSON.stringify(date_today)
              ) {
              ...
              } else console.log("Codeforces Data Already set");
            } catch (err) {
              console.log("Codeforces Error", err);
            }
          }
        );
      } else console.log("Codeforces Error");
    });
      ...
      if (data.status == "Success") {
        chrome.storage.sync.get(["date", "codechef"], stored_data => {
          try {
            ...
            if (
              stored_data.codechef == undefined ||
              JSON.stringify(stored_data.date) != JSON.stringify(date_today)
            ) {
            ...
            } else console.log("Codechef Data Already set");
          } catch (err) {
            console.log("Codechef Error", err);
          }
        });
      } else console.log("Codechef Error");
    });
      ...
      if (data.status == "Success") {
        chrome.storage.sync.get(["date", "spoj"], stored_data => {
          try {
            ...
            } else console.log("SPOJ Data Already set");
          } catch (err) {
            console.log("SPOJ Error", err);
          }
        });
      } else console.log("SPOJ Error");
    });
      ...
      if (data.status == "Success") {
        chrome.storage.sync.get(["date", "interviewbit"], stored_data => {
          try {
            ...
            // console.log('Stored Data', stored_data);
            if (
              stored_data.interviewbit == undefined ||
              JSON.stringify(stored_data.date) != JSON.stringify(date_today)
            ) {
            ...
            } else console.log("Interviewbit Data Already set");
          } catch (err) {
            console.log("Interviewbit Error", err);
          }
        });
      } else console.log("Interviewbit Error");
    });
}

function create_alarm() {
  chrome.alarms.get("Reset", alarm => {
    if (alarm) console.log("Alarm already exists", alarm);
    else {
      ...
      reset_time =
        new Date().setHours(reset_time_info.hours, reset_time_info.minutes) +
        86400000;
    }
  });
}

chrome.runtime.onInstalled.addListener(create_alarm);
chrome.runtime.onStartup.addListener(create_alarm);
